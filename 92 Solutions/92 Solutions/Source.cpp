...
bool PlaceQueen(int column) {
	if (column == NUMBER_COLS) { // Base Condition: compare vectors to see if you have already seen the solution, if not then continue
		...
		for (int row = 0; row < RowPlacement.size(); ++row) {
			// FIXME: complete as in EX8_2, but this time find all valid solutions!
